...
for(i in c(10, 1:9, 11:16)){
  ...
  lit_layer <- sapply(ntl.i[], FUN = function(x) ifelse(x > 0, 1, -1))
  #ntl.i[] <- log(1+ntl.i[])
  z.ntl_layer <- log(1+ntl.i[])
  #z.ntl_layer <- rep(NA, length(ntl.i[]))
  
  pop.i <- raster(paste("code_output/Population/GPW4_", yi, ".tif", sep = ""))
  #pop.i <- log(1+pop.i[])
  z.pop_layer <- pop.i[]/100000 #log(1+pop.i[])
  #z.pop_layer <- sapply(pop.i[], FUN = function(x) ifelse(x > 200000, 1, 0))
  ...
  z.ntl_file <- paste("code_output/z_covariates/z_ntl_", yi, sep = "")
  z.pop_file <- paste("code_output/z_covariates/z_pop_", yi, sep = "")
  ...
  writeRaster(z.pop_layer, z.pop_file, format = "GTiff", overwrite = TRUE)
