...
country_stats <- data.frame(year = sort(rep(2000:2015, length(afri_main$ISO3))),
                            ...
                            num_poppix = NA, #number of pixels with population
                            num_litpix = NA, #number of lit pixels
                            num_litpoppix = NA) #number of lit pixels with population
...
for(iso3j in afri_main$ISO3){
  ...
  for(i in 1:16){
    ...
    ntl <- raster(paste("data/ntl/GP2_Africa_5k/GP2_Africa_", min(yi, 2013) , ".tif", sep = ""))
