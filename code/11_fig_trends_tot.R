...
trends$left_y <- NA
trends$left_x <- NA
trends$left_y[mask00] <- (trends$pop_electricity/trends$pop_total)[mask00]
...
trends$right_y <- NA
trends$right_x <- NA
trends$right_y[mask13] <- (trends$pop_electricity/trends$pop_total)[mask13]
...
tt <- "total"
fig_gbox <- ggplot(subset(trends, type==tt), aes(year, 100 * pop_electricity/pop_total)) +
...
ggplot(subset(trends, type==tt & iso3=="GNQ"), aes(year, 100 * pop_electricity/pop_total)) +
            geom_line() 

ggplot(subset(trends, type==tt & iso3=="GNQ"), aes(year, 100 * pix_electricity)) +
            geom_line() 
